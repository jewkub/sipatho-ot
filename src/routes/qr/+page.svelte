<script lang="ts">
  ...
  onMount(async () => {
    const res = await fetch('/api/qr?id=' + (data.id || ''))
    qrUrl = await res.text()
    loading = false
    console.log(qrUrl)
  })
</script>
...
<div class="container p-8 h-screen w-full flex flex-col">
  ...
  {:else if qrUrl}
    <h2 class="text-center m-4 text-2xl">
      QR Code สำหรับฟอร์ม OT ประจำ{new Date().toLocaleDateString('th-TH', { dateStyle: 'full' })}
    </h2>
    <img class="object-contain flex-1 min-w-0 min-h-0" src={qrUrl} alt="qr" />
  {:else}
    <h2 class="text-center m-4 text-2xl">
